...
type Props = {
  data?: StudySession[]; // Made optional since it's not used
};
...
const SessionUI = ({}: Props) => {
  ...
  useEffect(() => {
    const now = moment(); // Current time (e.g., March 12, 2025)
    const planDate = currentStudyPlan?.date ? moment(currentStudyPlan.date) : now; // Plan date (e.g., March 11, 2025)
    ...
    currentStudyPlan?.schedule?.forEach((session) => {
      const [startStr, endStr] = session.time.split("-"); // Split on single dash
      const sessionStartTime = planDate.clone().set({
        hour: parseInt(startStr.split(":")[0]),
        minute: parseInt(startStr.split(":")[1]),
      });
      const sessionEndTime = planDate.clone().set({
        hour: parseInt(endStr.split(":")[0]),
        minute: parseInt(endStr.split(":")[1]),
      });

      if (now.isBetween(sessionStartTime, sessionEndTime)) {
